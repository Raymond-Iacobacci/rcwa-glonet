...
def make_propagator(params, f):
  ...
  # print(f"This is sampX and sampY, verify their similarity: {sampX}, {sampY}")
  # RIGHT HERE:
  k = tf.tile(k, multiples = (1, 2 * sampX - 1, 2 * sampY - 1)) # this is of shape sampx, sampy basically
  ...
  front = -front[::-1]
  # print(front.shape,k_xlist_pos.shape)
  k_xlist = np.hstack((front, k_xlist_pos))
  # print(k_xlist.shape)
  k_x = np.kron(k_xlist, np.ones((2 * sampY - 1, 1))) # Switched sampX and sampY in line 903
  # print(f"This is where the issue is, we want the k_x to only be defined for a certain range on y, it shouldn't be a square")
  # print(f"This is the shape: {k_x.shape}")
  k_x = k_x[np.newaxis, :, :]
  # Change def here, he knows what he did
  k_ylist_pos = 2 * np.pi * np.linspace(0, 1 / (2 *  params['Ly'] / params['upsample']), sampY)
  front = k_ylist_pos[-(sampY - 1):]
  front = -front[::-1]
  k_ylist = np.hstack((front, k_ylist_pos))
  k_y = np.kron(k_ylist, np.ones((2 * sampX - 1, 1)))
  k_y = k_y[np.newaxis, :, :]
  # k_y = np.transpose(k_x, axes = [0, 2, 1])
  # Flip just because easier with one line midway, doesn't want to alter before
  k_x = tf.convert_to_tensor(k_x, dtype = tf.complex64)
  # print(f"This is k_x's shape: {k_x.shape}")
  k_x = tf.tile(k_x, multiples = (batchSize, 1, 1))
  # print(f"This is k_y's shape: {k_y.shape}")
  k_y = tf.convert_to_tensor(k_y, dtype = tf.complex64)
  k_y = tf.tile(k_y, multiples = (batchSize, 1, 1))
  # print(f"This is the shape before transposing: {k_x.shape}")
  k_x = tf.transpose(k_x, perm = (0, 2, 1))
  # print(f"This is the shape after transposing: {k_x.shape}")
  k_z_base = tf.square(k_x) + tf.square(k_y)
  k_z_arg = tf.square(k) - (tf.square(k_x) + tf.square(k_y))
  k_z = tf.sqrt(k_z_arg)
  # print(f"This is the k_z that's messing us up: {k_z}")
...
def propagate(field, propagator, upsample):
  ...
  # print(f"This is the field shape after the transpose step: {field.shape}")
  field_real = tf.math.real(field)
  field_imag = tf.math.imag(field)
  # field_real = tf.image.resize(field_real, [n, n], method = 'nearest')
  field_real = tf.image.resize(field_real, [nx, ny], method = 'nearest')
  # field_imag = tf.image.resize(field_imag, [n, n], method = 'nearest')
  field_imag = tf.image.resize(field_imag, [nx, ny], method = 'nearest')
  ...
  field = tf.image.resize_with_crop_or_pad(field, 2 * nx - 1, 2 * ny - 1)
  field = tf.transpose(field, perm = [2, 0, 1])
# Just limit the fields and limit the propagator--it's not done correctly in his original code. TODO: recheck all other code to make sure no other issues, but as of now it's working
  # Alternatively:
  # //RAYMOND//
  # _,m,n=field.shape
  # n1 = upsample * m
  ...
  # field_imag=tf.math.imag(field)
  # d=max(n1,n2)
  # field_real=tf.image.resize_with_pad(field_real, d, d, method = 'nearest')
  # field_imag=tf.image.resize_with_pad(field_imag, d, d, method = 'nearest')
  # field=tf.cast(field_real,dtype=tf.complex64)+1j*tf.cast(field_imag,dtype=tf.complex64)
  # field=tf.image.resize_with_crop_or_pad(field, 2 * d - 1,2*d-1)
  # field=tf.transpose(field,perm=[2,0,1])

  # Apply the propagator in Fourier space.
  # print(f"This is the field shape: {field.shape}, and this is the propagator shape: {propagator.shape}")
  # print(f"This is the field shape before applying the fftshift function--correctness here implies that the issue is with line 982, or the application of the fftshift function: {field.shape}")
  ...
  out = tf.image.resize_with_crop_or_pad(out, nx, ny)
...
def simulate(ER_t, UR_t, params = initialize_params()):
  ...
  kinc_z0 = kinc_z0[:, :, :, 0, :, :] # Here, and search for kinc_x0 and kinc_y0 (searching for NLayer 0)

  # Unit vectors
  T1 = np.transpose([2 * np.pi / params['Lx'], 0])
  T2 = np.transpose([0, 2 * np.pi / params['Ly']]) # Not referenced from here
...
def solver_step5p1(L, Nlay_f, ERC, URC, k0, KX, KY, V0, W0, SG):
  # Convert int params back to correct type.
  Nlay = int(Nlay_f)
  # print(tf.linalg.inv(ERC).shape, "THAT WAS THE INVERSE")
  # Build the eigenvalue problem.
...
def solver_step5p3(L, Nlay_f, ERC, URC, k0, KX, KY, V0, W0, SG, LAM):
  ...
  # This is pixelsX * pixelsY matrix exponentials of size (p^2 * 2)x(q^2 * 2)
  #X = tf.linalg.expm(arg) # EXPENSIVE CALCULATION
  
  X0 = tf.linalg.expm(arg[:,:9,:,:,:])
...
solver_step5p3_recompute = tf.recompute_grad(solver_step5p3)
solver_step5p4_recompute = tf.recompute_grad(solver_step5p4)
solver_step5p5_recompute = tf.recompute_grad(solver_step5p5)
...
def simulate_allsteps(ER_t, UR_t, params): # This is just simulate with the steps wrapped up
  '''
    Calculates the transmission/reflection coefficients for a unit cell with a
    given permittivity/permeability distribution and the batch of input conditions 
    (e.g., wavelengths, wavevectors, polarizations) for a fixed real space grid
  ...
  ### Step 5: Calculate eigenmodes ###
  #S, SG = solver_step5(L, Nlay_f, ERC, URC, k0, KX, KY, V0, W0, SG)
  ...
  X = solver_step5p4(L, Nlay_f, ERC, URC, k0, KX, KY, V0, W0, SG, LAM, X0)
